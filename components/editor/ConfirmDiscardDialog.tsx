...
export function ConfirmDiscardDialog({ isOpen, onCancel, onDiscard, onSaveFirst }: ConfirmDiscardDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 p-4 backdrop-blur-sm">
      <div className="w-full max-w-md rounded-xl border border-border/80 bg-background shadow-[0_24px_60px_rgba(2,8,23,0.18)]">
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t px-6 py-5">
          ...
          <Button type="button" variant="outline" onClick={onDiscard}>저장하지 않고 이동</Button>
          <Button type="button" onClick={onSaveFirst}>저장 후 이동</Button>
        </div>
      </div>
    </div>
  )
